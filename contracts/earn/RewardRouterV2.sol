...
contract RewardRouterV2 is ReentrancyGuardUpgradeable {
    ...
    function signalTransfer(address _receiver) external nonReentrant {
        require(
            IERC20Upgradeable(bfrVester).balanceOf(msg.sender) == 0,
            "RewardRouter: sender has vested tokens"
        );
        require(
            IERC20Upgradeable(blpVester).balanceOf(msg.sender) == 0,
            "RewardRouter: sender has vested tokens"
        );
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        require(
            IERC20Upgradeable(bfrVester).balanceOf(_sender) == 0,
            "RewardRouter: sender has vested tokens"
        );
        require(
            IERC20Upgradeable(blpVester).balanceOf(_sender) == 0,
            "RewardRouter: sender has vested tokens"
        );
        ...
        uint256 esBfrBalance = IERC20Upgradeable(esBfr).balanceOf(_sender);
        if (esBfrBalance > 0) {
            IERC20Upgradeable(esBfr).transferFrom(_sender, receiver, esBfrBalance);
        }
    ...
    function _validateReceiver(address _receiver) private view {
        ...
        require(
            IERC20Upgradeable(bfrVester).balanceOf(_receiver) == 0,
            "RewardRouter: bfrVester.balance > 0"
        );
        require(
            IERC20Upgradeable(blpVester).balanceOf(_receiver) == 0,
